...
from soma_hub.evaluation import load_default_dataset, run_default_evaluation
from soma_hub.api import make_handler
from soma_hub.live_audio import macos_live_transcript_command, selected_audio_engine
...
class SomaHubTests(unittest.TestCase):
    def make_hub(self) -> tuple[tempfile.TemporaryDirectory, SomaHub]:
        temp = tempfile.TemporaryDirectory()
        return temp, SomaHub(Path(temp.name))
    ...
    def test_codec_reads_legacy_soma1_payload(self) -> None:
        temp, hub = self.make_hub()
        self.addCleanup(temp.cleanup)
        key = hub.codec._key
        ...
        token = "soma1:" + base64.urlsafe_b64encode(nonce + mac + ciphertext).decode("ascii")
    ...
    def test_detailed_memory_is_not_plaintext_in_sqlite(self) -> None:
        ...
        db_path = Path(temp.name) / "soma_hub.sqlite3"
    ...
    def test_spatial_word_map_persists_pose_and_world_reads_db(self) -> None:
        temp, hub = self.make_hub()
        self.addCleanup(temp.cleanup)
        db = Path(temp.name) / "soma_hub.sqlite3"
    ...
    def test_graph_profiles_api_route_uses_label_filter(self) -> None:
        ...
        handler.headers = {"X-SOMA-Token": "dev-token"}
        handler._json = Mock()
