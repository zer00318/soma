...
from soma_hub.capture_simulator import CaptureSimulator
from soma_hub.crypto import EncryptedTextCodec
from soma_hub.extraction import ExtractedInput, ExtractionPipeline
from soma_hub.graph import RelationalMemoryGraph
from soma_hub.platform_audio import live_audio_status
from soma_hub.recall import RecallFirewall
from soma_hub import importer
...
class SomaHub:
    def __init__(self, data_dir: Path) -> None:
        ...
        self.store = MemoryStore(data_dir / "soma_hub.sqlite3", self.codec)
        self.graph = RelationalMemoryGraph(data_dir / "soma_hub.sqlite3", self.codec)
    ...
    def import_whatsapp(self, export_path: str) -> dict:
        return importer.import_whatsapp(export_path, str(self.data_dir / "soma_hub.sqlite3"))

    def import_ics(self, ics_path: str) -> dict:
        return importer.import_ics(ics_path, str(self.data_dir / "soma_hub.sqlite3"))

    def import_imessage(self) -> dict:
        return importer.import_imessage(str(self.data_dir / "soma_hub.sqlite3"))
