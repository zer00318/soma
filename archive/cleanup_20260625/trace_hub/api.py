...
def make_handler(hub: TraceHub, token: str):
    class TraceHandler(BaseHTTPRequestHandler):
        server_version = "TRACEHub/0.1"
        ...
        def do_GET(self) -> None:
            ...
            if self.path == "/dashboard":
                ...
                html = (
                    f"<html><head><title>TRACE</title><style>"
        ...
        def _authorized(self) -> bool:
            return self.headers.get("X-TRACE-Token") == token
...
def run_server(host: str, port: int, data_dir: Path, token: str) -> None:
    hub = TraceHub(data_dir)
    server = ThreadingHTTPServer((host, port), make_handler(hub, token))
    print(f"TRACE hub listening on http://{host}:{port}")
    print("Use X-TRACE-Token for local app requests.")
    server.serve_forever()


def main() -> None:
    parser = argparse.ArgumentParser(description="Run the TRACE local hub API.")
    parser.add_argument("--host", default="0.0.0.0")  # LAN-accessible for iPhone pairing
    parser.add_argument("--port", type=int, default=8765)
    parser.add_argument("--data-dir", default="data")
    args = parser.parse_args()
    token = os.environ.get("TRACE_HUB_TOKEN", "dev-token")
