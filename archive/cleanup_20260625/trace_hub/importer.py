"""
TRACE content-extraction importer — populates the Tier-B relational graph
...
# Loaded from TRACE_USER_NAME env-var (space-separated); default "pranav".
_USER_NAME_TOKENS: frozenset[str] = frozenset(
    os.environ.get("TRACE_USER_NAME", "pranav").lower().split()
)
...
def _commitment_window_days() -> int:
    try:
        return int(os.environ.get("TRACE_COMMITMENT_WINDOW_DAYS", "60"))
    except ValueError:
        return 60
...
def _connect_graph(db_path: str) -> sqlite3.Connection:
    """Open the TRACE graph SQLite database.

    If the database does not yet have the TRACE schema (fresh file), initialize
    ...
    _graph = RelationalMemoryGraph(Path(db_path), EncryptedTextCodec(b"trace-importer-init-key-32bytes!!"))
...
def _is_user_sender(sender: str) -> bool:
    """True when every token of the sender name matches TRACE_USER_NAME tokens."""
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Pre-populate TRACE Tier-B graph from existing user data."
    )
    parser.add_argument("--db", required=True, help="Path to trace_hub.sqlite3")
