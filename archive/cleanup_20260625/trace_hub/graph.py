...
from trace_hub.location import PlaceCandidate, PlaceResolver
...
class RelationalMemoryGraph:
    ...
    def _upsert_event(self, conn: sqlite3.Connection, fact: PerceptionFact, captured_at: str, metadata: dict[str, Any]) -> str:
        ...
        # Deduplicate against capture time, not the host wall clock. Replay and
        # delayed ingestion otherwise turn every historical frame into a new event.
        dedup_row = conn.execute(
            """
            SELECT id, confidence, observation_count, status
            FROM graph_events
            WHERE event_type = ? AND status IN ('current', 'provisional')
              AND datetime(last_seen_at) BETWEEN datetime(?, '-5 minutes') AND datetime(?, '+5 minutes')
              AND json_extract(metadata_json, '$.location_hint') IS ?
            ORDER BY last_seen_at DESC LIMIT 1
            """,
            (event_type, captured_at, captured_at, location_hint or None),
        ).fetchone()
        if dedup_row:
        ...
        # Broader seven-day upsert for the same event type, also capture-relative.
        row = conn.execute(
            """
            SELECT id, confidence, observation_count, status
            FROM graph_events
            WHERE event_type = ? AND status IN ('current', 'provisional')
              AND datetime(last_seen_at) BETWEEN datetime(?, '-7 days') AND datetime(?, '+7 days')
            ORDER BY last_seen_at DESC LIMIT 1
            """,
            (event_type, captured_at, captured_at),
        ).fetchone()
        if row:
            observations = int(row["observation_count"]) + 1
            confidence = max(float(row["confidence"]), fact.confidence)
