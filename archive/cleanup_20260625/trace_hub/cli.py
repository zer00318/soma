...
from trace_hub.api import run_server
from trace_hub.evaluation import run_default_evaluation
from trace_hub.graph_audit import GraphAuditor, audit_to_dict
from trace_hub.live_audio import LiveAudioBridge, macos_live_transcript_command, selected_audio_engine
...
def main() -> None:
    parser = argparse.ArgumentParser(description="TRACE local hub")
    ...
    evaluate = sub.add_parser("evaluate", help="Run the built-in TRACE evaluation harness")
    ...
    if args.command == "serve":
        ...
        if args.json:
            ...
            print(f"TRACE graph audit: {result.grade} ({result.score}%)")
