...
onward), trace_hub/graph.py (LAW — patterns there are binding),
scripts/trace_demo_dashboard.py, SpatialWorld.swift, MobileCLIPNamer.swift.

Project law (violations = rejected work): X-TRACE-Token header;
data/trace_hub.sqlite3; `with self._connect()`; no Flask; never Apple
ID/password; Python suite (122) + `python3 evaluation/run_north_star.py`
(1.00) must be green after ANY trace_hub change; iOS build recipe =
...
- Every region whose best word fails the floors: append
  ...
  `Application Support/TRACE/vocab_misses.ndjson`.
