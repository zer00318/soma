...
Watches the worker's file queue (/tmp/soma_enrich) for crop/sidecar pairs and
...
In-process graph writes, no server — matches the phone-library architecture.
    ...
    .venv/bin/python -m soma_perception.enricher --db data/soma_hub.sqlite3
"""
...
def main() -> int:
    ...
    parser.add_argument("--db", default="data/soma_hub.sqlite3")
    parser.add_argument("--queue", default="/tmp/soma_enrich")
