...
@MainActor
final class SomaDetectorBridge: ObservableObject {
    static let shared = SomaDetectorBridge()
    ...
    func start() {
        ...
        let missing = SomaLocalRuntime.missingDetectorRequirements()
        ...
        let process = Process()
        process.executableURL = URL(fileURLWithPath: SomaLocalRuntime.detectorPython)
        process.currentDirectoryURL = URL(fileURLWithPath: SomaLocalRuntime.workspace)
        process.arguments = [
            "-m",
            "soma_perception.stream_worker",
            "--model",
            SomaLocalRuntime.detectorModel,
            "--confidence",
            "0.35",
        ]
