...
public class FastVLMProcessor: UserInputProcessor {
    ...
    public func prepare(prompt: UserInput.Prompt, imageTHW: THW?) -> String {
        func stringMessages(_ rawMessages: [Message]) -> [[String: String]] {
            rawMessages.map { message in
                [
        ...
        var messages = stringMessages(
            DefaultMessageGenerator().generate(from: UserInput(prompt: prompt.description))
        )
        switch prompt {
        case .text:
            break
        case .messages(let promptMessages):
            messages = stringMessages(promptMessages)
        case .chat(let chatMessages):
            messages = stringMessages(DefaultMessageGenerator().generate(messages: chatMessages))
        }
