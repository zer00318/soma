...
\chapter{Introduction and Problem Statement}\label{introduction-and-problem-statement}

\subsection{1.1 The problem: lives are forgotten by the people who live them}\label{the-problem-lives-are-forgotten-by-the-people-who-live-them}
...
\subsection{1.2 The problem statement}\label{the-problem-statement}
...
\subsection{1.3 The central hypothesis}\label{the-central-hypothesis}
...
\subsection{1.4 What SOMA is not}\label{what-soma-is-not}
...
\subsection{1.5 Contributions of this document}\label{contributions-of-this-document}
...
\subsection{1.6 How to read this document}\label{how-to-read-this-document}
...
\chapter{Conceptual Foundations}\label{conceptual-foundations}

\subsection{2.1 The standard model: a brain never touches the world}\label{the-standard-model-a-brain-never-touches-the-world}
...
\subsection{2.2 Episodic memory is reconstruction, not playback}\label{episodic-memory-is-reconstruction-not-playback}
...
\subsection{2.3 Sleep consolidation: why the binder runs at night}\label{sleep-consolidation-why-the-binder-runs-at-night}
...
\subsection{2.4 Event segmentation: why capture is gated, not uniform}\label{event-segmentation-why-capture-is-gated-not-uniform}
...
\subsection{2.5 Text as the retention format: the four arguments}\label{text-as-the-retention-format-the-four-arguments}
...
\subsection{2.6 The resolution of the central tension}\label{the-resolution-of-the-central-tension}
...
\chapter{Prior Art and Position}\label{prior-art-and-position}

\subsection{3.1 The memex lineage: total capture as an old dream}\label{the-memex-lineage-total-capture-as-an-old-dream}
...
\subsection{3.2 MyLifeBits and SenseCam: storage-first, and what it taught}\label{mylifebits-and-sensecam-storage-first-and-what-it-taught}
...
\subsection{3.3 Ego4D and egocentric vision: the demand curve, measured}\label{ego4d-and-egocentric-vision-the-demand-curve-measured}
...
\subsection{3.4 Retrieval-augmented generation: the ASK stage's ancestry}\label{retrieval-augmented-generation-the-ask-stages-ancestry}
...
\subsection{3.5 The commercial field: an empty quadrant}\label{the-commercial-field-an-empty-quadrant}
...
\subsection{3.6 Knowledge graphs and open vocabulary}\label{knowledge-graphs-and-open-vocabulary}
...
\chapter{The Seven Invariants}\label{the-seven-invariants}
...
\subsection{4.1 I1 --- No raw persistence}\label{i1-no-raw-persistence}
...
\subsection{4.2 I2 --- Text-only egress}\label{i2-text-only-egress}
...
Enforcement. EgressGuard.text() (in src/soma/application/egress\_guard.py) raises PrivacyViolationError for any payload that is not a TextEgress; the type's own constructor rejects empty text and empty provenance. The guard is eleven lines long --- short enough to audit in one breath, which is the point.

\subsection{4.3 I3 --- Append-only truth}\label{i3-append-only-truth}
...
\subsection{4.4 I4 --- Provenance everywhere}\label{i4-provenance-everywhere}
...
\subsection{4.5 I5 --- Calibrated confidence; refusal over fabrication}\label{i5-calibrated-confidence-refusal-over-fabrication}
...
Enforcement. Confidence rejects values outside {[}0,1{]} at construction; Binding.is\_usable(floor) is the refusal gate in domain form; the binder's contract (Chapter 10) makes refusal the default disposition of weak evidence; calibration itself is a measured property (Chapter 13), per {[}20{]}.

\subsection{4.6 I6 --- Unconstrained questions}\label{i6-unconstrained-questions}
...
\subsection{4.7 I7 --- Nothing is built until a missed question demands it}\label{i7-nothing-is-built-until-a-missed-question-demands-it}
...
\subsection{4.8 The invariants as a system}\label{the-invariants-as-a-system}
...
\chapter{The Pipeline: From Photons to Answers}\label{the-pipeline-from-photons-to-answers}
...
\subsection{5.1 Stage 1 --- Senses}\label{stage-1-senses}
...
\subsection{5.2 Stage 2 --- Extraction}\label{stage-2-extraction}
...
\subsection{5.3 Stage 3 --- The macro picture}\label{stage-3-the-macro-picture}
...
\subsection{5.4 Stage 4 --- The helpers}\label{stage-4-the-helpers}
...
\subsection{5.5 Stage 5 --- Text}\label{stage-5-text}
...
\subsection{5.6 Stage 6 --- The binder}\label{stage-6-the-binder}
...
\subsection{5.7 Stage 7 --- Node creation}\label{stage-7-node-creation}
...
\subsection{5.8 Stage 8 --- The information supplier}\label{stage-8-the-information-supplier}
...
\subsection{5.9 What flows between stages}\label{what-flows-between-stages}
...
\chapter{The Crow: One Memory, End to End}\label{the-crow-one-memory-end-to-end}
...
\subsection{6.1 Tuesday, 07:42:03 --- the eyes fire}\label{tuesday-074203-the-eyes-fire}

You step onto the balcony carrying a trash bag. A crow lands on the railing. The always-on detector --- cheap, running on every gated frame --- registers a dark object entering the scene and holds a track on it. It emits: observation: kind=object, subject=black bird, attributes={[}position: balcony\_railing, size: small{]}, t=07:42:03.210, spatial\_anchor=balcony/railing, confidence=0.71, provenance=(ev-4183, channel=detector) The frame that produced this observation is already gone. No pixel of the crow exists anywhere.

\subsection{6.2 Tuesday, 07:42:04 --- the ears fire, independently}\label{tuesday-074204-the-ears-fire-independently}

The sound tagger, a separate always-on helper that knows nothing of the detector, hears a harsh call: observation: kind=sound\_event, subject=corvid call (caw), t=07:42:04.030, spatial\_anchor=balcony, confidence=0.66, provenance=(ev-4184, channel=audio) Two lines of text now exist that happen to share a second and a place. Nothing yet says they are the same event. That ignorance is deliberate: capture channels never coordinate; coordination is the night's job. Cheap parallel senses first, expensive fusion later --- the same decomposition biological perception uses.

\subsection{6.3 Tuesday, 07:42:06 --- attention pays for one deep look}\label{tuesday-074206-attention-pays-for-one-deep-look}

The bird stays. In the scheduler (Chapter 9), the track's dwell accumulates while its novelty is high --- a bird on this railing is not familiar --- so salience crosses the enrichment threshold and the token bucket has budget. One token is spent; the expensive eye looks once, at just this crop, at level 1 of the enrichment ladder: observation: kind=vlm\_gist, subject=large corvid, attributes={[}color: glossy black, activity: perched, watching{]}, t=07:42:06.400, confidence=0.78, provenance=(ev-4188, channel=vlm) Meanwhile the sensors never stopped: pose says balcony, facing east (ev-4189); the detector's other track says a trash bag is in hand (ev-4190). Five observations, three channels, one shared scaffold of when-and-where. Total persistent footprint of the entire event: under a kilobyte of text.

\subsection{6.4 Tuesday, 23:40 --- the night shift binds}\label{tuesday-2340-the-night-shift-binds}

You sleep; the device charges; the big model reads the day. The binder clusters by co-occurrence: ev-4183 (dark shape, railing, 07:42:03), ev-4184 (caw, balcony, 07:42:04), ev-4188 (corvid gist, 07:42:06) --- same place, same seconds, compatible kinds. It proposes an entity and scores the join: entity: crow\_1, kind=animal, label=crow --- binding: subject=crow\_1, predicate=observed\_at, object=balcony/railing @ 07:42, confidence=0.87, evidence=(ev-4183, ev-4184, ev-4188) The crow node is born: a living cell citing every observation it grew from. Note what the confidence is doing --- 0.87 is not decoration, it is the survival threshold made explicit. Which matters immediately, because:

\subsection{6.5 Tuesday, 23:40:02 --- and refuses what it cannot prove}\label{tuesday-234002-and-refuses-what-it-cannot-prove}
...
\subsection{6.6 Wednesday and Friday --- the node thickens}\label{wednesday-and-friday-the-node-thickens}

Wednesday 07:44: trash bag, crow, caw. Friday 07:39: trash bag, crow. Each morning writes fresh observations; each night the binder finds the same shape in them. Identity resolution matters here: the new sightings do not spawn crow\_2 and crow\_3 --- same place, same behavioral signature, same kind resolve to crow\_1, whose confidence rises to 0.93 and whose evidence tuple now spans three days. Nodes that fire together, wire together; nodes that recur, thicken.

\subsection{6.7 Friday, 23:42 --- 1 + 1 = 4}\label{friday-2342-1-1-4}

Now the cell combines with its fellow nodes and derives something no sensor ever observed. The pattern engine notices: every observation cluster containing crow\_1 also contains the trash-bag-in-hand observation, three mornings out of three, within the same two-minute window: derived binding: subject=crow\_1, predicate=co\_occurs\_with, object=trash-run (morning routine), confidence=0.81, evidence=(tue cluster, wed cluster, fri cluster) One crow plus one chore equals four: the crow, the chore, the pattern, and the anticipation the pattern licenses. This is the napkin's arithmetic made literal, and it is the entire justification for building a graph instead of a diary: isolated observations answer ``what happened?''; combined nodes answer ``why?'' and ``what will?''.

\subsection{6.8 Saturday, 10:15 --- you ask}\label{saturday-1015-you-ask}

``Why is that crow always on my balcony?'' The supplier walks the graph --- crow\_1, its sightings, its derived habit --- and hands the reasoner a dossier of perhaps twenty lines. The answer returns with its evidence attached: ``A crow has been landing on your balcony railing on the mornings you take the trash out --- seen Tuesday, Wednesday, and Friday around 07:40. It appears to be showing up for the trash.'' --- citations: ev-4183, ev-4188, wed cluster, fri cluster ``What color were its eyes?'' The supplier finds no observation about eyes --- the level-1 enrichment never got that deep, because dwell never earned level 3. The grounding gate finds the draft unsupported, and the memory answers: ``I didn't perceive that.'' No fabrication. The refusal is not a failure of the system; it is the system, working. A storage-first product would have offered to re-watch the video. SOMA has no video --- it has the honesty of its words.
...
\subsection{6.9 What the example proves}\label{what-the-example-proves}
...
\chapter{The Domain Model}\label{the-domain-model}
...
\subsection{7.1 Observation --- the atom}\label{observation-the-atom}

An Observation is one typed emission from one perceiver: an open-vocabulary kind and subject; a tuple of Attribute name--value pairs; capture time t\_ms; an optional spatial\_anchor; a Confidence; a Provenance; and an optional refutation\_cue --- a hint about what evidence would disprove the observation, falsifiability designed into the datum itself. Constructors reject empty kinds and subjects, negative times, and --- critically --- any disagreement between the observation's time and its provenance's capture time: an observation cannot claim to be from a moment its evidence trail denies.

\subsection{7.2 Provenance --- the birth certificate}\label{provenance-the-birth-certificate}

Provenance is three fields: event\_id, source\_channel, captured\_at\_ms, each validated non-empty/non-negative. It answers, for any datum in the system, the auditor's three questions: which event, which sense, which millisecond. Nothing exists without one (I4).

\subsection{7.3 Confidence --- a number with a contract}\label{confidence-a-number-with-a-contract}
...
\subsection{7.4 Entity and Binding --- the graph}\label{entity-and-binding-the-graph}

Entity is a stable identity: id, kind, label. Binding is the edge: subject--predicate--object where the object is exactly one of an entity reference or free text (the constructor rejects both-or-neither); a Confidence; and a non-empty tuple of Provenance evidence. Binding.is\_usable(minimum\_confidence) is the refusal gate expressed in domain form --- one comparison, but placed where every consumer must pass through it.

\subsection{7.5 Query, Citation, RecallAnswer --- the ASK contract}\label{query-citation-recallanswer-the-ask-contract}
...
\subsection{7.6 TextEgress --- the only exportable thing}\label{textegress-the-only-exportable-thing}

TextEgress is non-empty text plus a non-empty tuple of source\_event\_ids. It is the single type the device boundary accepts (I2), and its constructor makes anonymous egress impossible (I4): text cannot leave without naming the observations it came from.

\subsection{7.7 Why constructors, not validators}\label{why-constructors-not-validators}

All seven types validate in \textbf{post\_init} and are frozen thereafter. The alternative --- permissive construction plus downstream validation --- distributes the invariant over every consumer and guarantees eventual drift. Here, an invalid domain object cannot exist, so every function accepting an Observation inherits its guarantees silently. The strict-typing gate (Chapter 14) runs mypy --strict over this layer with 100\% of the public surface typed; the test suite covers the constructor rejections explicitly.
...
\subsection{8.1 Hexagonal: ports and adapters}\label{hexagonal-ports-and-adapters}
...
\chapter{The Perception Subsystem}\label{the-perception-subsystem}
...
\subsection{9.1 The cheap stratum: always-on helpers}\label{the-cheap-stratum-always-on-helpers}
...
\subsection{9.2 Salience: deciding what deserves the expensive eye}\label{salience-deciding-what-deserves-the-expensive-eye}

The scheduler (soma\_perception/scheduler.py) is pure logic --- no I/O, injected clock, portable 1:1 to Swift. Its inputs are track statistics from the cheap stratum; its outputs are sparse, budgeted enrichment decisions. The scoring function: salience = dwell\_progress \(\times\) (0.6 \textperiodcentered{} novelty + 0.4 \textperiodcentered{} stability) Dwell is attention's signature --- what the wearer lingers near matters; novelty front-loads the unfamiliar --- the hundredth sighting of the kitchen kettle is worth less than the first crow; stability suppresses motion blur --- a deep look at a smear is a wasted token. The weights are starting points, tuned against captured data, not doctrine.

\subsection{9.3 The budget: a token bucket with no overdraft}\label{the-budget-a-token-bucket-with-no-overdraft}
...
\subsection{9.4 The progressive enrichment ladder}\label{the-progressive-enrichment-ladder}

Depth of attention buys depth of knowledge, in three levels --- overview \(\rightarrow\) attributes \(\rightarrow\) minutiae --- where each level costs one token and demands roughly 3\(\times\) the accumulated dwell of the previous: dwell time \(\rightarrow\) knowledge depth \(\rightarrow\) L1 overview (costs 1 token, \textasciitilde3\(\times\) dwell) L2 attributes (costs 1 token, \textasciitilde3\(\times\) dwell) L3 minutiae (costs 1 token, \textasciitilde3\(\times\) dwell) Figure 8 --- The progressive enrichment ladder: attention depth buys knowledge depth. Level 1 records what a glance records: category, color, activity. Level 2 is a delta pass --- the prompt includes what the graph already knows and asks only for new detail: material, condition, readable text, accessories. Level 3 hunts identifying minutiae: scratches, stickers, wear marks, unique blemishes --- the details that make this backpack findable among the world's backpacks. Delta prompting matters twice over: it saves tokens, and it structurally prevents the model from re-asserting (and re-hallucinating) what is already known. A worked scheduler trace The crow's enrichment decision, with the arithmetic visible. At 07:42:06 the track has been alive \textasciitilde3 seconds; nothing like it has been seen at this anchor before; the bird is still. Input Value Note dwell\_progress (toward L1 threshold) 0.75 3s of a 4s L1 requirement novelty 0.9 first corvid at this anchor stability (1 - motion) 0.8 perched; low blur risk salience

\subsection{\texorpdfstring{0.75 \(\times\) (0.6\textperiodcentered{}0.9 + 0.4\textperiodcentered{}0.8) = 0.65}{0.75 \textbackslash times (0.60.9 + 0.40.8) = 0.65}}\label{times-0.60.9-0.40.8-0.65}

above the 0.5 decision floor bucket tokens

\subsection{2.3 available}\label{available}

refill 6/hr, last spend 22 min ago decision enrich, level 1, focus ``overview'' one token spent Contrast the kitchen kettle at 09:15: dwell\_progress 1.0 (stared at while waiting), but novelty \(\approx\) 0.05 after a hundred sightings --- salience 0.35, below the floor, no token spent. The kettle stays a cheap word; the crow earned a deep look. That asymmetry, repeated thousands of times a day, is the whole attention economy. The prompts encode field-learned humility --- ``material and brand are often NOT determinable from an image\ldots{} a confident wrong material is worse than none'' --- a lesson bought with a real error (a glass phone recorded as plastic) and now enforced in the prompt contract, including the permission to answer exactly: nothing new.

\subsection{9.5 The enricher: expensive eye, delta facts}\label{the-enricher-expensive-eye-delta-facts}
...
\subsection{9.6 Honesty about the current frontier}\label{honesty-about-the-current-frontier}
...
\chapter{The Binder: Consolidation as Sleep}\label{the-binder-consolidation-as-sleep}

\subsection{10.1 The job}\label{the-job}
...
\subsection{10.2 The contract}\label{the-contract}
...
\subsection{10.3 Accelerator, not gatekeeper}\label{accelerator-not-gatekeeper}
...
\subsection{10.4 The devil's advocate, kept in the room}\label{the-devils-advocate-kept-in-the-room}
...
\subsection{10.5 Status}\label{status}
...
\chapter{Recall: Grounded Answering with Calibrated Refusal}\label{recall-grounded-answering-with-calibrated-refusal}

\subsection{11.1 The loop}\label{the-loop}
...
\subsection{11.2 Just enough context}\label{just-enough-context}
...
A dossier for ``why is that crow always on my balcony?'', in full: ENTITY crow\_1 (animal, ``crow'') conf .93 --- evidence spans tue/wed/fri. B-0917 crow\_1 observed\_at balcony/railing tue 07:42 conf .87 {[}ev-4183, ev-4184, ev-4188{]} B-0954 crow\_1 observed\_at balcony/railing wed 07:44 conf .89 {[}ev-5211, ev-5214{]} B-1033 crow\_1 observed\_at balcony/railing fri 07:39 conf .90 {[}ev-7042{]} B-1035 crow\_1 co\_occurs\_with trash-run (morning routine) conf .81 {[}tue, wed, fri clusters{]} CONTEXT trash-run: object ``trash bag, in hand'' tue 07:42 {[}ev-4190{]}, wed 07:43 {[}ev-5213{]}, fri 07:38 {[}ev-7040{]} NEGATIVE no crow observations at other anchors; no observations of feeding.
...
\subsection{11.5 The text-egress hatch}\label{the-text-egress-hatch}
...
\chapter{The Privacy Architecture}\label{the-privacy-architecture}
...
ON DEVICE, EPHEMERAL \textperiodcentered{} raw frames (ms lifetime) \textperiodcentered{} raw audio (ms lifetime) \textperiodcentered{} no record path exists PERSISTED / EGRESSABLE \textperiodcentered{} TextEgress: text + source\_event\_ids \textperiodcentered{} append-only log, AES-256-GCM \textperiodcentered{} auditable, human-readable PERCEPTION + EGRESS GUARD any non-text payload at the wall \(\rightarrow\) PrivacyViolationError Figure 5 --- The one-way membrane: meaning crosses, media dies at the wall.

\subsection{12.1 Layer 1 --- Ephemerality at the source}\label{layer-1-ephemerality-at-the-source}
...
\subsection{12.2 Layer 2 --- The typed boundary}\label{layer-2-the-typed-boundary}
...
\subsection{12.5 The threat model, tabulated}\label{the-threat-model-tabulated}
...
\subsection{12.6 Regulatory alignment}\label{regulatory-alignment}
...
\subsection{12.7 What architecture cannot discharge}\label{what-architecture-cannot-discharge}
...
\chapter{Evaluation: Every Miss Names the Next Build}\label{evaluation-every-miss-names-the-next-build}
...
\subsection{13.1 RAS: fabrication priced like a loss, because it is one}\label{ras-fabrication-priced-like-a-loss-because-it-is-one}
...
\subsection{13.2 OAG: the price of throwing the pixels away}\label{oag-the-price-of-throwing-the-pixels-away}
...
\subsection{13.3 The (A)/(B) instrument}\label{the-ab-instrument}
...
\subsection{13.4 The method: sufficiency, then necessity, then efficiency}\label{the-method-sufficiency-then-necessity-then-efficiency}
...
\subsection{13.5 The current honest baseline}\label{the-current-honest-baseline}
...
\subsection{13.6 Anti-overfit machinery}\label{anti-overfit-machinery}
...
\subsection{13.7 The self-play flywheel}\label{the-self-play-flywheel}
...
\chapter{Engineering Governance}\label{engineering-governance}
...
\subsection{14.1 The merge gate}\label{the-merge-gate}
...
\subsection{14.2 One source of truth, read first, updated last}\label{one-source-of-truth-read-first-updated-last}
...
\subsection{14.3 The plan-gate and the anti-diversion law}\label{the-plan-gate-and-the-anti-diversion-law}
...
\subsection{14.4 Honesty as an artifact}\label{honesty-as-an-artifact}
...
\chapter{Market Position and the Moat}\label{market-position-and-the-moat}

\subsection{15.1 The quadrant}\label{the-quadrant}
...
\subsection{15.2 The demo that proves the moat}\label{the-demo-that-proves-the-moat}
...
\subsection{15.3 Why incumbents want it and cannot build it}\label{why-incumbents-want-it-and-cannot-build-it}
...
\chapter{Risks and Open Problems}\label{risks-and-open-problems}

Stated plainly, ordered by severity, with the mitigation that exists and the trigger that would escalate each.

\subsection{16.1 The central hypothesis may be false}\label{the-central-hypothesis-may-be-false}
...
\subsection{16.2 Hallucination is at 27.3\% against an invariant of \textasciitilde0}\label{hallucination-is-at-27.3-against-an-invariant-of-0}
...
\subsection{16.3 Local models may be the wall}\label{local-models-may-be-the-wall}
...
\subsection{16.4 The live loop is unproven on device}\label{the-live-loop-is-unproven-on-device}
...
\subsection{16.5 Overfit to the hero capture}\label{overfit-to-the-hero-capture}
...
\subsection{16.6 Bystander and social risk}\label{bystander-and-social-risk}
...
\subsection{16.7 The binder as single point of intellectual failure}\label{the-binder-as-single-point-of-intellectual-failure}
...
\subsection{17.1 Prototype tiers}\label{prototype-tiers}
...
\subsection{17.2 Phases and gates}\label{phases-and-gates}
...
2 --- Close (A)s, kill hallucination build only the channels missed questions demand (egomotion first); tighten refusal until made-up \(\approx\) 0 hallucination \textasciitilde0; spatial answered or honestly refused 3 --- Close (B)s navigation/indexing where context was present but unused; measured hatch if needed no unexplained (B) failures on the set 4 --- Necessity \& harden ablate to load-bearing channels; moat-proof panel; cold-capture guard anti-overfit green; minimal channel set known 5 --- Package demo script + moat proof + market beat, rehearsed end to end founder runs the pitch alone

\subsection{17.3 The founder's rituals}\label{the-founders-rituals}
...
\chapter{Objections and Responses}\label{objections-and-responses}
...
\subsection{18.1 ``If you throw away the pixels, you'll throw away the}\label{if-you-throw-away-the-pixels-youll-throw-away-the}
...
\subsection{18.2 ``A verbal description of a scene is a pale shadow of the}\label{a-verbal-description-of-a-scene-is-a-pale-shadow-of-the}
...
\subsection{18.3 ``People will not wear a camera, period.''}\label{people-will-not-wear-a-camera-period.}
...
\subsection{18.4 ``Local models are too weak to bind a day correctly.''}\label{local-models-are-too-weak-to-bind-a-day-correctly.}
...
\subsection{18.5 ``The graph will silt up with garbage over months.''}\label{the-graph-will-silt-up-with-garbage-over-months.}
...
\subsection{18.6 ``Why not just embeddings? Vector search over frames is}\label{why-not-just-embeddings-vector-search-over-frames-is}
...
\subsection{18.7 ``OCR/ASR errors will poison the memory.''}\label{ocrasr-errors-will-poison-the-memory.}
...
\subsection{18.8 ``This is just RAG with extra steps.''}\label{this-is-just-rag-with-extra-steps.}
...
\subsection{18.9 ``The salience scheduler will miss the moment that}\label{the-salience-scheduler-will-miss-the-moment-that}
...
\subsection{18.10 ``Encryption with the vendor holding no key means no}\label{encryption-with-the-vendor-holding-no-key-means-no}
...
\subsection{18.11 ``Two people cannot build this.''}\label{two-people-cannot-build-this.}
...
\subsection{18.12 ``Why would anyone pay for honest refusals?''}\label{why-would-anyone-pay-for-honest-refusals}
...
\chapter*{Addendum --- Revision 1.1}\label{addendum-revision-1.1}
\addcontentsline{toc}{chapter}{Addendum --- Revision 1.1}

\emph{Added after version 1.0 was rendered: what building the desk pillar taught, which the design could not have known in advance.}

\chapter{The desk pillar's gaze economy: attention as an auction}\label{the-desk-pillars-gaze-economy-attention-as-an-auction}
...
\subsection{18.1 The organs}\label{the-organs}

The eye is not a camera pipeline; it is a gaze. A \textbf{retina} holds the raw
...
\textbf{Peripheral vision} watches the complete stream cheaply and continuously,
emitting only two facts: \emph{something changed here} and \emph{this scene has been
still for N seconds} --- and it measures its own true coverage rate rather than
assuming it, so the system's blind spots are numbers, not hopes. A \textbf{fovea}
...
\subsection{18.2 The auction}\label{the-auction}

Every consumer of perception declares a two-sided \textbf{diet contract}: what it
...
the hard way and is now enforced by a test: \textbf{no specialist is architecture}.
...
\subsection{18.3 The dwell ladder and the Nutella law}\label{the-dwell-ladder-and-the-nutella-law}

A scene that persists earns \emph{deepening}, never repetition. L0 glances (what is
...
only the delta against a \textbf{novelty ledger} --- a still screen over minutes
...
\subsection{18.4 The ceiling}\label{the-ceiling}

Extraction quality is measured against a frontier model's maximum extraction
from the same scene --- the \textbf{ceiling}. A miss shared by product and ceiling
...
banked frame corpus, is \textbf{0.026}. The frontier also serves, during
...
\subsection{18.5 The live-only theorem}\label{the-live-only-theorem}

Because raw dies at the membrane (I1) and the night shift eats text only, any
relation visible \emph{only} in raw material --- motion correlation, sound direction,
...
\chapter{Laws the cell taught us: identity, birth, and night}\label{laws-the-cell-taught-us-identity-birth-and-night}

The graph of Chapter 7 is typed evidence; the desk pillar's node layer put a
\emph{living} cell on top of it --- born, warmed, combined, pruned. Building it
surfaced laws that belong in this document precisely because none of them were
in the design.

\subsection{19.1 Identity never merges under uncertainty}\label{identity-never-merges-under-uncertainty}

Person nodes are \textbf{instances, not names}: two Sams remain two nodes forever.
Anonymous people are \textbf{per-context ghosts}, never one global bucket. And the
...
the resolver attaches \emph{nothing} and a question carries the fork to the owner,
naming both candidates. Answered questions land as verbatim truth and heal the
graph mechanically.

\subsection{19.2 Ubiquity cannot vouch}\label{ubiquity-cannot-vouch}
...
level: a \emph{place} may not be the via-node of a co-occurrence chain, because
everything co-occurs at home --- ``A and B are both often at home'' is trivia
wearing the costume of a derivation.

\subsection{19.3 Newborns are born warm}\label{newborns-are-born-warm}
...
the most recently \emph{thought} thing in the graph and is born warm. Its rent
...
\subsection{19.4 Contradiction fires only on functional keys}\label{contradiction-fires-only-on-functional-keys}
...
is now scoped to \emph{functional} keys --- attributes for which two values are
...
\subsection{19.5 The night, mapped to its biology}\label{the-night-mapped-to-its-biology}

The night shift runs an economy in fixed order: succession (temporal edges),
schema refit (routines crystallize; deviation becomes surprise), \textbf{mining} ---
...
\chapter*{Appendix W --- Witnessed status of the desk pillar (2026-07-20)}\label{appendix-w-witnessed-status-of-the-desk-pillar-2026-07-20}
\addcontentsline{toc}{chapter}{Appendix W --- Witnessed status of the desk pillar (2026-07-20)}
...
\tightlist
\item
  Suite: \textbf{274 passed, 1 xfailed} --- the single xfail is the VLM specialist
  eye, gated by the one-GPU law, not by missing work.
\item
  \textbf{The core loop has run end-to-end on lived data}: live screen \(\rightarrow\) auction
...
\item
  Perception bank: \textbf{181 golden frames} auto-captured during the founder's
  normal work; \textbf{111 frontier ceiling extractions} over \textbf{4,674 claims};
  L1-verbatim gap to ceiling \textbf{0.026}.
\item
...
\item
  The build map: \textbf{489 tracked parts, 43 open design questions}, every part
...
Internal primary sources (SOMA repository) ops/NORTH\_STAR.md --- product source of truth: the three-stage machine, the attention principle, metrics, prototype tiers, honest critiques.

ops/ROADMAP.md --- canonical implementation spec: central hypothesis, the (A)/(B) instrument, sufficiency\(\rightarrow\)necessity\(\rightarrow\)efficiency, phases and gates, the anti-diversion law. ops/MARKET\_AND\_SCOPE.md --- quadrant analysis, named acquirers, scope boundaries. ops/enrichment\_scheduler\_design.md --- salience scheduler design note (Swift portability contract).

docs/PHASE0\_REPORT.md --- verified Phase 0 state: gate output, coverage, known debt, contamination flags.
...
{[}41{]} A. L. Yarbus, \emph{Eye Movements and Vision}, Plenum Press, 1967.

{[}42{]} R. P. N. Rao and D. H. Ballard, ``Predictive coding in the visual cortex: a functional interpretation of some extra-classical receptive-field effects,'' \emph{Nature Neuroscience} 2(1), 79--87, 1999.

{[}43{]} A. M. Collins and E. F. Loftus, ``A spreading-activation theory of semantic processing,'' \emph{Psychological Review} 82(6), 407--428, 1975.

{[}44{]} J. L. McClelland, B. L. McNaughton, and R. C. O'Reilly, ``Why there are complementary learning systems in the hippocampus and neocortex,'' \emph{Psychological Review} 102(3), 419--457, 1995.

{[}45{]} M. A. Wilson and B. L. McNaughton, ``Reactivation of hippocampal ensemble memories during sleep,'' \emph{Science} 265, 676--679, 1994.

{[}46{]} S. Diekelmann and J. Born, ``The memory function of sleep,'' \emph{Nature Reviews Neuroscience} 11, 114--126, 2010.

{[}47{]} J. O'Keefe and L. Nadel, \emph{The Hippocampus as a Cognitive Map}, Oxford University Press, 1978.

{[}48{]} E. F. Loftus and J. C. Palmer, ``Reconstruction of automobile destruction: an example of the interaction between language and memory,'' \emph{Journal of Verbal Learning and Verbal Behavior} 13, 585--589, 1974.

{[}49{]} K. Friston, ``The free-energy principle: a unified brain theory?'', \emph{Nature Reviews Neuroscience} 11, 127--138, 2010.

{[}50{]} S. A. Jelbert, A. H. Taylor, L. G. Cheke, N. S. Clayton, and R. D. Gray, ``Using the Aesop's Fable paradigm to investigate causal understanding of water displacement by New Caledonian crows,'' \emph{PLoS ONE} 9(3), e92895, 2014.

{[}23{]}{[}24{]}{[}25{]} supplies the three patterns that structure the monolith: event sourcing (the

{[}23{]}{[}24{]}{[}25{]} supplies the three patterns that structure the monolith: event sourcing (the

\chapter*{Appendix A --- The Napkin, Transcribed and Mapped}\label{appendix-a-the-napkin-transcribed-and-mapped}
\addcontentsline{toc}{chapter}{Appendix A --- The Napkin, Transcribed and Mapped}
...
\subsection{A.1 The Standard Model block}\label{a.1-the-standard-model-block}
...
\subsection{A.2 The EXECUTION block, phrase by phrase}\label{a.2-the-execution-block-phrase-by-phrase}
...
\subsection{D.3 The walk baseline, as a worked scorecard}\label{d.3-the-walk-baseline-as-a-worked-scorecard}
...
\chapter*{Appendix E --- Annotated Bibliography}\label{appendix-e-annotated-bibliography}
\addcontentsline{toc}{chapter}{Appendix E --- Annotated Bibliography}

The bibliography, re-read with intent: for each cluster of sources, what it establishes and which SOMA decision rests on it.

\subsection{E.1 Memory science}\label{e.1-memory-science}
...
\subsection{E.2 Lifelogging and egocentric vision}\label{e.2-lifelogging-and-egocentric-vision}
...
\subsection{E.3 Language models, retrieval, and honesty}\label{e.3-language-models-retrieval-and-honesty}
...
\subsection{E.4 Software architecture}\label{e.4-software-architecture}
...
\subsection{E.5 Models and systems}\label{e.5-models-and-systems}
...
\subsection{E.6 Privacy and law}\label{e.6-privacy-and-law}
...
\chapter*{Appendix F --- The Question Battery, in Full Shape}\label{appendix-f-the-question-battery-in-full-shape}
\addcontentsline{toc}{chapter}{Appendix F --- The Question Battery, in Full Shape}
...
\chapter{Category Example question What it stresses 1--4 Object identity ``What brand was my backpack?''}\label{category-example-question-what-it-stresses-14-object-identity-what-brand-was-my-backpack}
...
\chapter*{Appendix G --- The Decision Log}\label{appendix-g-the-decision-log}
\addcontentsline{toc}{chapter}{Appendix G --- The Decision Log}
...
Decision Rationale Revisit when \(\blacklozenge\) Text-only retention (I1) privacy structural; text queryable/auditable sufficiency run falsifies losslessness (\S{}16.1) \(\blacklozenge\) Unconstrained questions (I6) constrained surface hides hallucination never --- supersedes north star's older suggestion \(\blacklozenge\) Refusal over fabrication (I5) trust is the product never \(\blacksquare\) MobileCLIP-S0 for naming ANE-fast; words-not-prose; open vocab; audit: weak link was proposals, not naming desk-test diag shows naming (not proposals) is the bottleneck \(\blacksquare\) ARKit for pose/reloc only free 6-DOF + relocalization on the hardware cross-day reloc fails at its measured gate \(\blacksquare\) Append-only SQLite event log replay, audit, crash-safety; no server store outgrows device (not projected) \(\blacksquare\) AES-256-GCM user-key codec authenticated encryption; versioned AAD crypto review mandates change \(\blacksquare\) Strangler over rewrite for ask\_home 2,900 lines of earned heuristics; rewrite risk strangling completes (celebration, not revisit) \(\blacksquare\) No app rewrite before demo build+install loop is the only device-test loop after the pitch window \(\blacklozenge\) Kill the 3D world renderer no question needed it (I7's founding case) a diagnosed (A) spatial miss that egomotion cannot close \(\blacksquare\) Gemma-3-12B local / hatch to frontier text local-first economics; typed egress makes hatch safe measured local-vs-hatch delta justifies switch \(\blacklozenge\) RAS \(\geq\) 60, hallucination \textless{} 5\% as prototype gate the stranger-amazement bar made numeric founder resets the bar THE DECISION LOG

\chapter*{Appendix H --- A Day in the Life of the System}\label{appendix-h-a-day-in-the-life-of-the-system}
\addcontentsline{toc}{chapter}{Appendix H --- A Day in the Life of the System}
...
23:30 --- the night shift. Charger connected; thermals irrelevant; the big model wakes. Congregate, dedupe, bind by co-occurrence; refuse the weak links (the day's coincidences die here); resolve identities (today's crow is crow\_1, thickened, not crow\_4); run the derivation pass (three trash-morning clusters become a habit node); rebuild the projections; run the self-play battery against the day and file the misses into the (A)/(B) queue. Toward dawn: the discarder drops the marinated structures the night's better judgment replaced. The raw material it never touches.
...
\chapter*{Appendix I --- The Capture Notation: Worked Records}\label{appendix-i-the-capture-notation-worked-records}
\addcontentsline{toc}{chapter}{Appendix I --- The Capture Notation: Worked Records}
...
\subsection{I.1 A detector observation}\label{i.1-a-detector-observation}

Field Value kind object subject black bird attributes (position: balcony\_railing), (size: small), (motion: landing) t\_ms 27\_723\_210 (07:42:03.210) spatial\_anchor balcony/railing confidence 0.71 provenance (event\_id: ev-4183, source\_channel: detector, captured\_at\_ms: 27\_723\_210) refutation\_cue track lost within 1s \(\rightarrow\) possible shadow The refutation cue deserves a note: it is the observation announcing, at birth, what future evidence would disprove it. A shadow misread as a bird is expected to lose its track within a second; when the track instead persists forty seconds and earns a VLM look, the cue's condition fails and the observation's standing strengthens. Falsifiability is not a philosophy here; it is a field.

\subsection{I.2 A sound observation}\label{i.2-a-sound-observation}

Field Value kind sound\_event subject corvid call (caw) attributes (loudness: moderate), (direction: east) t\_ms 27\_724\_030 spatial\_anchor balcony confidence 0.66 provenance (ev-4184, audio, 27\_724\_030)

\subsection{I.3 An enrichment observation (level 1)}\label{i.3-an-enrichment-observation-level-1}

Field Value kind vlm\_gist subject large corvid attributes (color: glossy black), (activity: perched, watching), (level: overview) t\_ms 27\_726\_400 spatial\_anchor balcony/railing confidence 0.78 provenance (ev-4188, vlm, 27\_726\_400)

\subsection{I.4 A binding, after the night shift}\label{i.4-a-binding-after-the-night-shift}

Field Value binding\_id b-0917 subject\_id crow\_1 predicate observed\_at object\_text balcony/railing @ tue 07:42 confidence 0.87 evidence (ev-4183, detector), (ev-4184, audio), (ev-4188, vlm) Note the constructor discipline visible even in a table: the binding has object\_text and therefore no object\_id (exactly one, enforced); it has three evidence entries (non-empty, enforced); its confidence is a probability (range-checked at construction). A malformed memory is not a bug to find later --- it is an object that never existed.

\subsection{I.5 The whole minute, as storage}\label{i.5-the-whole-minute-as-storage}
...
\chapter*{Appendix J --- Substrates and Hardware Budgets}\label{appendix-j-substrates-and-hardware-budgets}
\addcontentsline{toc}{chapter}{Appendix J --- Substrates and Hardware Budgets}
...
\subsection{J.1 V0 --- the Mac}\label{j.1-v0-the-mac}
...
\subsection{J.2 V1 --- the phone rig}\label{j.2-v1-the-phone-rig}
...
\subsection{J.3 Glasses --- the destination}\label{j.3-glasses-the-destination}
...
\subsection{J.4 The budget table}\label{j.4-the-budget-table}
...
\chapter*{Appendix K --- Future Work}\label{appendix-k-future-work}
\addcontentsline{toc}{chapter}{Appendix K --- Future Work}
...
The engineer (an afternoon). Chapters 4--5 (invariants, pipeline) \(\rightarrow\) Chapters 7--11 (domain to recall) \(\rightarrow\) Appendix C (the formal surface) \(\rightarrow\) Appendix I (the wire format) \(\rightarrow\) then the repository itself, in the order README \(\rightarrow\) ops/NORTH\_STAR.md \(\rightarrow\) ops/ROADMAP.md \(\rightarrow\) src/soma/domain/.
...
\chapter*{Appendix M --- The Legacy Engine: An Archaeology and a Strangling Plan}\label{appendix-m-the-legacy-engine-an-archaeology-and-a-strangling-plan}
\addcontentsline{toc}{chapter}{Appendix M --- The Legacy Engine: An Archaeology and a Strangling Plan}

scripts/ask\_home.py --- roughly 2,900 lines --- is the system's functioning ancestor: the answer engine that produced every baseline number in Chapter 13. It predates the domain model, violates most of the style gate, and contains 47 broad exception handlers. It is also a repository of earned answer-quality defenses, bought one hallucination at a time on real captures, and the strangling plan's first rule is that none of them may be lost in translation.

\subsection{M.1 What it knows that a rewrite would forget}\label{m.1-what-it-knows-that-a-rewrite-would-forget}
...
\subsection{M.2 The strangling sequence}\label{m.2-the-strangling-sequence}

Capability Legacy home New home Status Answer boundary + citations ad-hoc dict output Recall \(\rightarrow\) RecallAnswer via LegacyAskHome done Bound memory bound\_memory (empty, unusable) Binder consensus projection first real version on follow-on branch Grounded generation inline prompt rules GroundedRecall on local gemma first version on follow-on branch Claim verification scattered regex gates grounding\_gate.py (Phase 3) boundary named Retrieval / indexes per-script builders projections over the event log migrating State/phantom/absence defenses prompt strings + regexes typed gate rules with tests to extract The migration's covenant: every extracted capability lands behind a port with tests that encode the legacy behavior it must preserve, and the baseline battery re-runs after each extraction --- the strangler fig with a regression harness wrapped around it. When the last capability moves, the 47 broad excepts retire with the file, not before.

\chapter*{Appendix N --- Threat Scenarios, Walked}\label{appendix-n-threat-scenarios-walked}
\addcontentsline{toc}{chapter}{Appendix N --- Threat Scenarios, Walked}
...
\chapter*{Appendix O --- Binding Arithmetic: How 0.87 Happens}\label{appendix-o-binding-arithmetic-how-0.87-happens}
\addcontentsline{toc}{chapter}{Appendix O --- Binding Arithmetic: How 0.87 Happens}

The crow's binding confidence, decomposed. The binder scores a candidate join over evidence features; the shape below is the contract (weights illustrative, tuned per Chapter 13's method, never hand-trusted): Feature Crow join (ev-4183 + ev-4184 + ev-4188) Chime join (ev-4191 + crow\_1) Temporal proximity (window overlap)

\subsection{0.95 --- within 3.2s}\label{within-3.2s}

\subsection{0.71 --- within 7s}\label{within-7s}

Spatial agreement (anchor match)

\subsection{\texorpdfstring{0.9 --- railing \(\subset\) balcony}{0.9 --- railing \textbackslash subset balcony}}\label{railing-subset-balcony}

\subsection{0.4 --- chime anchored next-door}\label{chime-anchored-next-door}

Kind compatibility (object\(\leftrightarrow\)sound\(\leftrightarrow\)gist priors)

\subsection{\texorpdfstring{0.85 --- corvid call \(\leftrightarrow\) bird \(\leftrightarrow\)}{0.85 --- corvid call \textbackslash leftrightarrow bird \textbackslash leftrightarrow}}\label{corvid-call-leftrightarrow-bird-leftrightarrow}

corvid gist

\subsection{\texorpdfstring{0.2 --- chime \(\leftrightarrow\) animal: no prior}{0.2 --- chime \textbackslash leftrightarrow animal: no prior}}\label{chime-leftrightarrow-animal-no-prior}
