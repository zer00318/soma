...
TASK_QUEUE = [
    {
        "id": "instance_categorize",
        "helper": "instance_graph",
        "title": "Categorize instances by VLM identity, not detector label",
        "description": (
            "In scripts/instance_graph.py, the function `_categorize(name, text, det_label)` "
            "is a STUB that just returns the detector label. Implement it properly.\n\n"
            "The detector (Grounding-DINO) often mislabels: a chip bag detected as 'blanket', "
            "a phone as 'monitor', ketchup as 'jar bottle'. But the strong VLM read the REAL "
            "identity into `name`/`text` (e.g. 'Lays Xtra Flamin Hot', 'Apple iPhone', 'Heinz "
            "Tomato Ketchup'). Your job: return a coarse CATEGORY that prefers the identity.\n\n"
            "Rules:\n"
            "- Build a keyword map from brand/identity words to categories, e.g.: lays, "
            "doritos, pringles, chips, tortilla, crisps -> 'snack'; iphone, macbook, samsung, "
            "galaxy, laptop, monitor, phone, ipad -> 'electronics'; heinz, ketchup, mustard, "
            "mayo, sauce -> 'condiment'; coke, pepsi, water, juice, soda -> 'beverage'; "
            "nutella, jam, bread, cheese -> 'food'; shoe, shirt, adidas, nike, pants -> 'clothing'.\n"
            "- Check `name` and `text` (lowercased) against the map FIRST; if a keyword "
            "matches, return that category.\n"
            "- Only if neither name nor text gives any category signal, fall back to the "
            "cleaned detector label (lowercased, strip leading 'a '/'an '/'the ').\n"
            "- Keep the exact signature `def _categorize(name: str, text: str, det_label: str) -> str:`.\n"
            "Make tests/test_instance_graph.py pass."
        ),
        "read_file": "scripts/instance_graph.py",
        "verify": [VENV_PY, "-m", "pytest", "tests/test_instance_graph.py", "-q"],
        "verify_timeout": 60,
    },
    {
        "id": "salience_tiering",
        "helper": "salience_helper",
        "title": "Tier salience notes (observed / recognized / inferred)",
        "description": (
            "In scripts/salience_helper.py, the function `tier_salience(note: str) -> str` "
            "is a STUB that always returns 'observed'. Implement it.\n\n"
            "It classifies one 'what's interesting here' note into a tier so the brain can "
            "fence it and never report a guess as fact:\n"
            "- 'recognized': the note identifies a SPECIFIC named entity by appearance — a "
            "proper-noun person (e.g. 'Kushal Mehra'), or a known brand/logo (e.g. 'Adidas "
            "logo', 'the Eiffel Tower'). Recognition takes PRIORITY over hedging: 'looks "
            "like the Eiffel Tower' is 'recognized', because it names a specific entity.\n"
            "- 'inferred': the note is hedged/speculative ('maybe', 'looks like', 'might', "
            "'could be', 'possibly', 'seems') AND names no specific entity (e.g. 'looks like "
            "a coffee stain').\n"
            "- 'observed': a concrete visible detail or anomaly stated plainly, no hedge "
            "(e.g. 'visible dust on the keyboard keys'). This is the default.\n\n"
            "Implementation hints: detect a specific entity via a capitalized multi-word "
            "proper noun, or a small known-brand keyword set (adidas, nike, apple, eiffel, "
            "etc.). Detect hedging via a hedge-word set. Order: check recognized first, then "
            "inferred, else observed. Keep the exact signature. Make "
            "tests/test_salience_helper.py pass."
        ),
        "read_file": "scripts/salience_helper.py",
        "verify": [VENV_PY, "-m", "pytest", "tests/test_salience_helper.py", "-q"],
        "verify_timeout": 60,
    },
    {
        "id": "salience_entity",
        "helper": "salience_helper",
        "title": "Extract the entity name from a recognized salience note",
        "description": (
            "In scripts/salience_helper.py, implement the STUB `extract_entity(note) -> str`. "
            "It pulls the bare entity name from a 'recognized' note for world-knowledge lookup: "
            "'appears to be the philosopher Kushal Mehra' -> 'Kushal Mehra'; 'recognised the "
            "Adidas logo' -> 'Adidas'; 'looks like the Eiffel Tower in the background' -> "
            "'Eiffel Tower'; plain anomaly notes -> ''. Strip lead-ins (appears to be, looks "
            "like, recognised, the, a/an, role words like philosopher/author) and trailing "
            "words (logo, in the background). Keep the capitalized proper-noun / brand span; "
            "return '' if the note names no specific entity. Make tests/test_salience_entity.py pass."
        ),
        "read_file": "scripts/salience_helper.py",
        "verify": [VENV_PY, "-m", "pytest", "tests/test_salience_entity.py", "-q"],
        "verify_timeout": 60,
    },
    {
        "id": "graph_count_by_category",
        "helper": "instance_graph",
        "title": "Count instances grouped by category",
        "description": (
            "In scripts/instance_graph.py, implement the STUB `count_by_category(graph) -> dict`. "
            "Group node counts by each node's 'category' field (set by _categorize), so 'how "
            "many snacks' answers across 'Lays' and 'Doritos' (both 'snack'). Return "
            "{category: count} like {'snack': 2, 'electronics': 1}. Skip nodes whose category "
            "is empty or 'object'. Make tests/test_graph_count.py pass."
        ),
        "read_file": "scripts/instance_graph.py",
        "verify": [VENV_PY, "-m", "pytest", "tests/test_graph_count.py", "-q"],
        "verify_timeout": 60,
    },
    # --- BRAIN WIRING: connect instance pipeline to the brain server ---
    {
        "id": "wire_perceive_and_graph",
        "helper": "instance_brain_wire",
        "title": "Wire perceive_and_graph: run instance pipeline + store graph",
        "description": (
            "In scripts/instance_brain_wire.py, implement the STUB "
            "`perceive_and_graph(frame_path, moment, graphs_store) -> dict`.\n\n"
            "The module already imports `instance_perceive` and `instance_graph`. "
            "Your job:\n"
            "1. Call `instance_perceive.perceive(frame_path)` — returns a list of instance "
            "dicts (each has box, det_label, name, text, colour, material, state, orient).\n"
            "2. Call `instance_graph.build_graph(instances)` — returns a dict with nodes, "
            "edges, counts.\n"
            "3. Store the result: `graphs_store[moment] = graph`.\n"
            "4. Return the graph dict.\n\n"
            "Let exceptions propagate (the caller handles errors). Keep the exact signature.\n"
            "Make tests/test_instance_brain_wire.py pass."
        ),
        "read_file": "scripts/instance_brain_wire.py",
        "verify": [VENV_PY, "-m", "pytest", "tests/test_instance_brain_wire.py", "-q",
                   "-k", "test_perceive"],
        "verify_timeout": 60,
    },
    {
        "id": "wire_answer_from_graph",
        "helper": "instance_brain_wire",
        "title": "Wire answer_from_graph: answer questions from stored instance graph",
        "description": (
            "In scripts/instance_brain_wire.py, implement the STUB "
            "`answer_from_graph(question, moment, graphs_store, ollama_host, model) -> dict | None`.\n\n"
            "The module already imports `instance_graph`. Your job:\n"
            "1. Look up `graphs_store.get(moment)`. If missing or has zero nodes, return None.\n"
            "2. Call `instance_graph.render_graph(graph)` — returns a text summary.\n"
            "3. Call `instance_graph.answer(graph, question)` — returns the LLM answer string.\n"
            "4. Check if the answer is a refusal: contains 'I don\\'t have' or "
            "'don\\'t have that' (case-insensitive). Set refused=True if so.\n"
            "5. Return: {\"answer\": str, \"source\": \"instance_graph\", \"refused\": bool, "
            "\"graph_nodes\": len(graph[\"nodes\"]), \"graph_edges\": len(graph[\"edges\"])}.\n\n"
            "Keep the exact signature. Make tests/test_instance_brain_wire.py pass."
        ),
        "read_file": "scripts/instance_brain_wire.py",
        "verify": [VENV_PY, "-m", "pytest", "tests/test_instance_brain_wire.py", "-q",
                   "-k", "test_answer"],
    ...
    {
        "id": "wire_graph_to_text",
        "helper": "instance_brain_wire",
        "title": "Wire graph_to_text_record: render graph as flat text for kf_memory",
        "description": (
            "In scripts/instance_brain_wire.py, implement the STUB "
            "`graph_to_text_record(graph) -> str`.\n\n"
            "The module already imports `instance_graph`. Your job:\n"
            "1. If graph has no nodes (graph[\"nodes\"] is empty), return \"\".\n"
            "2. Otherwise call `instance_graph.render_graph(graph)` and return the result.\n\n"
            "Keep the exact signature. Make tests/test_instance_brain_wire.py pass."
        ),
        "read_file": "scripts/instance_brain_wire.py",
        "verify": [VENV_PY, "-m", "pytest", "tests/test_instance_brain_wire.py", "-q",
