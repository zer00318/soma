#!/usr/bin/env python3
"""SOMA night shift — local-LLM task runner with verification gates.
...
them safely while no Claude session is running:
  ...
  - kind "founder": writes the request to /tmp/soma_founder_request.txt and
                    fires a macOS notification; the founder does the IRL
                    step and replies:  echo done > /tmp/soma_founder_reply.txt
...
logged to /tmp/soma_night_shift.log and state to ops/night_shift_state.json.

Run:  nohup python3 scripts/soma_night_shift.py > /dev/null 2>&1 &
Stop: pkill -f soma_night_shift
"""
...
SYSTEM_RULES = """You are a careful implementation worker on the SOMA codebase.
    ...
    for task in queue["tasks"]:
        ...
        if not ok:
            ...
            if task.get("blocking", True):
                notify("SOMA night shift blocked", f"{tid} failed — see log")
                log("blocking task failed — stopping shift (tree left green)")
                return
    notify("SOMA night shift complete", "All tasks finished")
