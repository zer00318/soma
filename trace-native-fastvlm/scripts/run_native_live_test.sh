...
APP_PATH="${TRACE_NATIVE_APP:-$HOME/Library/Developer/Xcode/DerivedData/FastVLM-hgpowjcsgrivjgftwcetohnqmdug/Build/Products/Debug/FastVLM App.app}"
LOG_FILE="${TRACE_NATIVE_LOG:-$HOME/Library/Application Support/Trace/trace_native_text.ndjson}"
NODE_BIN="${NODE_BIN:-$HOME/.cache/codex-runtimes/codex-primary-runtime/dependencies/node/bin/node}"
SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
REPORT_FILE="${TRACE_NATIVE_REPORT:-$WORKSPACE/trace-native-fastvlm/TRACE_NATIVE_LIVE_REPORT.md}"
...
This script clears TRACE's text log before each run. It does not save audio, video, images, screenshots, or frames.
USAGE
}
...
launch_clean() {
  ...
  pkill -f "trace_perception" >/dev/null 2>&1 || true
  pkill -f "whisper-stream" >/dev/null 2>&1 || true
  rm -f "$LOG_FILE"
  open "$APP_PATH"
  countdown 90 "Warming up TRACE native app"
}
...
case "$MODE" in
  ...
  ocr)
    ...
    ;;
  speech)
    prompt_phase 45 "Speak near the Mac: 'TRACE test pencil backpack thermal camera.'"
  ...
  full)
    ...
    verify --require-ocr
    prompt_phase 45 "Phase 3: speak near the Mac: 'TRACE test pencil backpack thermal camera.'"
    countdown 30 "Waiting for local Whisper text commit"
    verify --require-speech
    ;;
esac

printf '\nTRACE native live test passed: %s\n' "$MODE"
printf 'Text log: %s\n' "$LOG_FILE"
