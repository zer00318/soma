...
This is the native iOS/macOS direction for TRACE, forked from Apple `ml-fastvlm/app`.

Current state:

- Apple FastVLM camera + MLX app scaffold is present.
- FastVLM 0.5B native model is downloaded into `FastVLM/model`.
- The default prompt has been changed from visual Q&A to TRACE object/event memory.
