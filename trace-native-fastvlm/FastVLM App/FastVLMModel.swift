...
class FastVLMModel {
    ...
    // Heavy on-device VLM (the macro helper). FastVLM parrots its prompt and cannot
    // perceive; we ship Qwen2.5-VL-3B-4bit BUNDLED in FastVLM/model/ (loaded by the
    // bundled MLXVLM Qwen25VL implementation via config.json's model_type=qwen2_5_vl).
    // modelConfiguration points at that local bundle dir — NO network download, ever.
    private let modelConfiguration = FastVLM.modelConfiguration

    /// parameters controlling the output
    let generateParameters = GenerateParameters(temperature: 0.0)
    let maxTokens = 384  // room for a rich macro description (FastVLM's 96 starved it)
