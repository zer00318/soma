...
2. "YOUR ACTION" box: read /tmp/trace_founder_request.txt if non-empty and
   newer than /tmp/trace_founder_reply.txt, show it big; else "nothing
   needed from you".
3. WORKERS table: Codex (pgrep codex; STATE running/idle; last 5 log
   ...
   /tmp/trace_gates.txt if present: "tests OK/FAIL north-star X").
4. Phone: parse /tmp/trace_latest_spatial_pull.txt path, show object count
