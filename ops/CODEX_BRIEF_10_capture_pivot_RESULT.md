...
- `Sync to Mac` posts metadata only to `<somaHubURL>/capture/perception` with `X-SOMA-Token: dev-token`, `source=capture_session`, and `memory_text` shaped as `CAPTURE | <ts> | <duration>min | <size>MB`. File transfer remains devicectl pull.
