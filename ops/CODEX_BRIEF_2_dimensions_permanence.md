...
- Hub side (python, soma_hub/graph.py is LAW — read before touching):
...
- Every region whose best word scored BELOW threshold: append
  {ts, best_word, score} to Application Support/SOMA/vocab_misses.ndjson.
...
X-SOMA-Token; soma_hub.sqlite3; `with self._connect()`; no Flask; build
...
north-star eval after ANY soma_hub change (must stay green); write
