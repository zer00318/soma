...
| P11 | [Appearance fingerprints](P11-appearance-fingerprints.md) | guided | Opus medium | P00 | IN-PROGRESS(chief) — MAC SUBSTRATE LANDED 2026-07-04: cosine same-thing helper + typed storage + legacy-path flow + synthetic ROC (8 tests, battery held). Swift emission NOT shipped (deliberate — new per-frame ANE path must be measured not to tax perception; featureprint 2048>512-cap → model choice pending a device check). Then one walk proves P10+P11 together |
