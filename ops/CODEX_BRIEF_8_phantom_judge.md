...
(verification path), soma_hub/graph.py (LAW — its patterns are binding),
scripts/trace_demo_dashboard.py (/api/world).

Project law: X-SOMA-Token; data/soma_hub.sqlite3; `with self._connect()`;
no Flask; never Apple ID/password; Python suite + north-star
(python3 -m unittest discover -s tests -p "test_*.py";
python3 evaluation/run_north_star.py) MUST be green after any soma_hub
...
## P2 — "where is X" recall intent (Python, soma_hub)
